...
export class UserRepository {
   ...
   * @returns
   */
  static async getUserDetails(userId: string, type?: string) {
    const user = await prisma.user.findFirst({
      where: {
        id: userId,
        type: type,
   ...
   * @returns
   */
  static async createSuAdminUser({ name, username, email, password }) {
    try {
  ...
  static async createUser({
    ...
    try {
      ...
      if (driver_id) {
        data['driver_id'] = driver_id; // <-- THIS WAS MISSING
      }
