...
export class StringHelper {
  ...
  public static formatSearchQuery(query: string): string {
    // Sanitize the query by replacing characters that are not alphanumeric,
    // hyphens, or whitespace with a space. This prevents tsquery syntax errors.
    const sanitizedQuery = query.replace(/[^a-zA-Z0-9\-\s]/g, ' ');

    // Replace spaces with '|' for OR logic in tsquery
